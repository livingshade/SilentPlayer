...
fn load_current_if_needed<B: AudioBackend>(
    ...
    let gain = session
        .current_gain()
        .unwrap_or_else(|| GainDecision::unity(domain::LoudnessStatus::NeedsAnalysis));
    backend.load(
        track,
...
fn errors_from_playback(error: PlaybackError) -> PlayerError {
...
mod tests {
    ...
    fn track(name: &str) -> Track {
        ...
        track.loudness = Some(domain::LoudnessInfo::track(-20.0, -8.0));
        track
    }
